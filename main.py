...
ICO_LIST_URL = 'https://icobazaar.com/v2/ico-list'
ICO_LIST_PAGINATION_URL = 'https://icobazaar.com/v2/ico-list?cats%5B%5D={0}&page={1}'
...
def get_cats_documents():
    return mongo.find({}, 'cats_icobazaar')


def parse_cat(documents):
    ...
    while len(documents) > 0:
        ...
        while True:
            ...
            bs = BeautifulSoup(get_html(url), 'lxml')
            ...
            count += 1
            url = ICO_LIST_PAGINATION_URL.format(documents[0]['cat_num'], count)
            print(url)
        # break


def parse_all_cats():
    bs = BeautifulSoup(get_html(ICO_LIST_URL), 'lxml')
    cats = bs.findAll('a', {'class': 'filter-seo-link'})

    # * Вынести в отдельную функцию в утилиты.
    data = []
    cat_num = 0
    ...
    [mongo.insert_one(item, 'cats_icobazaar') for item in data]
    mongo.finish()


def main():
    parse_all_cats()
    parse_cat(get_cats_documents())
